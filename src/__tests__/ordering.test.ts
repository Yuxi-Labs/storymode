...
import { parseNarrative, Diagnostic } from '../storymodeIndex.js';
...
describe('Narrative ordering (spec draft placeholders)', () => {
  ...
  expect(res.diagnostics.some((d: Diagnostic) => d.code === 'NARRATIVE_METADATA_FORBIDDEN')).toBe(true);
  });
  ...
  it('Late character media cue not yet enforced', () => {
    const src = `::narrative: intro\n::scene: s1\n🞶 Ken\n↠ \"First line\"\n⦿ late_sfx`;
    const res = parseNarrative(src);
  expect(res.diagnostics.some((d: Diagnostic) => d.code === 'MEDIA_CUE_AFTER_DIALOGUE')).toBe(false);
  });
});
